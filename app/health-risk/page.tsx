...
export default function HealthRiskPage() {
  ...
  const handleFormSubmit = async (formData: any) => {
    setIsLoading(true)
    try {
      const response = await fetch("/api/predict-health-risk", {
        method: "POST",
      ...
      console.log("[v0] Full prediction response:", prediction)

      setResult({
        riskLevel: prediction.riskLevel || "Unknown",
        message: prediction.message || "Assessment completed",
      })
    } catch (error) {
          ...
          <Card className="shadow-lg">
            <CardContent className="pt-6">
              <div className="text-center space-y-4">
                <h2 className="text-3xl font-bold text-primary">Assessment Result</h2>
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-background via-white to-accent/20 py-12 px-4">
      <div className="w-full max-w-3xl mx-auto">
        ...
        <HealthRiskForm onSubmit={handleFormSubmit} isLoading={isLoading} />
      </div>
    </main>
  )
}
